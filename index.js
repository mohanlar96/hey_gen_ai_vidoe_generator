...
rl.question("What's your topic to Generate Video ? ", (topic) => {
  generateQA(topic)
    .then(async (scenes) => {
      console.log(scenes);
      console.log(
        `Successfully generated ${scenes.length} Q&A pairs about ${topic}:`
      );
...
Strickly follow the avatarName defined in the Example below.
  ...
  try {
    ...
    if (typeof text === "string") {
      try {
        ...
        console.error("Parsing error: Invalid JSON.", jsonErr);
        throw new Error("Failed to parse JSON from API response.");
      }
    } else {
      qaList = text; // already an object
    }
    ...
    if (
      ...
      console.warn(
        "Warning: The generated JSON might not perfectly match the expected structure."
      );
    }
    ...
    throw new Error(
      `Failed to generate QA for topic "${topic}". Details: ${apiError.message}`
    );
  }
}
